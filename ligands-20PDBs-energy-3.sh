...
#for y in PR/PR_agonist/targets/1SQN PR/PR_antagonist/targets/2OVH RAR_alpha/RAR_alpha_agonist/targets/3KMR RAR_alpha/RAR_alpha_antagonist/targets/1DKF ROR_gamma/ROR_gamma_agonist/targets/3L0L ROR_gamma/ROR_gamma_antagonist/targets/3B0W RXR_alpha/RXR_alpha_agonist/targets/1MV9 RXR_alpha/RXR_alpha_antagonist/targets/3NSQ 
	...
	more energy | while read x; do a=` echo $x | awk '{print $1}'`; b=`grep -m 1 $a ${nr}_dict `; if [[ $b ]]; then echo -n "$x "; echo $b; else echo -n "$x ";  grep -m 1 $a ~/scratch/NRs/dict/dict; fi; done  > energy-4 &
